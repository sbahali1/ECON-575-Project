...
acf(resid(pooled_model_best), title="Pooled Model ACF")

pre_covid_model = ardl(real_personal_income ~ avg_hourly_wage + pce_inflation + cpi_food + 
                          cpi_energy + u6,
                        data = pre_covid, order = c(1,2,1,2,0,1))
summary(pre_covid_model)
# acf(resid(pre_covid_model), main = "Pre-Covid Subspace ACF")
covid_model = ardl(real_personal_income ~ avg_hourly_wage + pce_inflation + cpi_food + 
                          cpi_energy + u6,
                        data = covid, order = c(1,2,1,2,0,1))
summary(covid_model)
# acf(resid(covid_model), main="Covid Subspace ACF")
