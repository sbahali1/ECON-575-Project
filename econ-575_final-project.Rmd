...
data <- read.csv("df_overall_economic_indicator_2012_2022.csv")
pre_covid = data[1:96,]
covid = data[97:123,]
```
...
avg_hourly_wages_p = ts(pre_covid$avg_hourly_wages, start = c(2012,1,1), frequency = 12)
...
real_personal_income_p = log(real_personal_income_p)
real_gdp_p = ts(na.remove(pre_covid$real_gdp), start = c(2012,1,1), frequency = 4)
public_debt_p = ts(na.remove(pre_covid$public_debt), start = c(2012,1,1), frequency = 4)
public_debt_p = log(public_debt_p)
```
...
library(gap)
library(olsrr)
lin_reg_all_features = lm(real_personal_income_p ~ black_ur_p + u6_p + ur_p + pce_inflation_p + avg_hourly_wages_p + cpi_food_p + cpi_energy_p)
ols_step_best_subset(lin_reg_all_features)
