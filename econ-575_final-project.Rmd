...
#cpi_combined = ts(c(cpi_food_p, cpi_energy_p), start = start(cpi_food_p), frequency = frequency(cpi_food_p))
#cpi_combined_2 = rbind(cpi_food_p, cpi_energy_p)
...
```{r}
library(gap)
pre_covid_model_reg <- auto_ardl(real_personal_income ~ avg_hourly_wage + pce_inflation + cpi_food + 
                          cpi_energy + u6,
                        data = pre_covid, 
                        max_order = 2)
pre_covid_model_reg$top_orders
pre_covid_model_best <- pre_covid_model_reg$best_model
summary(pre_covid_model_best)
acf(resid(pre_covid_model_best))

covid_model_reg <- auto_ardl(real_personal_income ~ avg_hourly_wage + pce_inflation + cpi_food +
                          cpi_energy + u6,
                        data = covid,
                         max_order = 2)
covid_model_reg$top_orders
covid_model_best <- covid_model_reg$best_model
summary(covid_model_best)
acf(resid(covid_model_best))

#Using model specification, we will perform chow test to see if coefficients are significantly different, indicating a break in the nature of personal income due to COVID-19
# covid_model = ardl(real_personal_income ~ avg_hourly_wage + pce_inflation + cpi_food + 
#                           cpi_energy + u6, data=covid, order = c(1, 2, 1, 0 ,0, 2))
summary(covid_model)
pooled_model_reg = auto_ardl(real_personal_income ~ avg_hourly_wage + pce_inflation + cpi_food + 
                          cpi_energy + u6,
                        data = data, 
                        max_order = 2)
...
library(qpcR)
covidRSS = RSS(covid_model_best)
pre_covidRSS = RSS(pre_covid_model_best)
pooledRSS = RSS(pooled_model_best)
F_stat = ((pooledRSS - (covidRSS + pre_covidRSS))/12)/((covidRSS + pre_covidRSS)/(121-(2*12)))
#Df = k, n-2k = 12, 97
